...
SceneTransforms.prototype.calculatePerspective = function(){
    ...
    mat4.perspective(30, gl.viewportWidth / gl.viewportHeight, 1.0, 1000.0, this.pMatrix);
};
...
SceneTransforms.prototype.updatePerspective = function(){
    mat4.perspective(65, gl.viewportWidth / gl.viewportHeight, 1.0, 1000.0, this.pMatrix);  // We can resize the screen at any point so the perspective matrix should be updated always.
};
...
SceneTransforms.prototype.setMatrixUniforms = function(){
	this.calculateNormal();
	if(this.prg != skyPrg){
    gl.uniformMatrix4fv(this.prg.mvMatrixUniform, false, this.mvMatrix);  //Maps the Model-View matrix to the uniform shaderProgram.uMVMatrix
    gl.uniformMatrix4fv(this.prg.pMatrixUniform, false, this.pMatrix);    //Maps the Perspective matrix to the uniform shaderProgram.uPMatrix
    gl.uniformMatrix4fv(this.prg.nMatrixUniform, false, this.nMatrix);  

	}
	else{
		
	gl.uniformMatrix4fv(this.prg.uMVMatrix, false, this.mvMatrix);  //Maps the Model-View matrix to the uniform shaderProgram.uMVMatrix
    gl.uniformMatrix4fv(this.prg.uPMatrix, false, this.pMatrix);    //Maps the Perspective matrix to the uniform shaderProgram.uPMatrix
    gl.uniformMatrix4fv(this.prg.uNMatrix, false, this.nMatrix);
	//console.log("Here");
	}//Maps the Normal matrix to the uniform shaderProgram.uNMatrix
};
